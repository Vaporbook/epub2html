...
module.exports.parse = function parse(epubfile, cb)
{
	...
	/* passed directly down from epub-cache and/or epub2web */
...
module.exports.convertMetadata = function convertMetadata(epubData)
{
	...
	try {
		htmlNav = processNavMap(epubData);
		htmlMetas = processMetas(epubData);
		htmlMetaList = processMetasAsList(epubData);
	} catch (e) {
		;
	}
...
module.exports.getParser = function getFiles()
{
	return parser;
}
...
function processMetas(epubData) {
	...
	for(prop in metas) {
				if(prop == 'meta') {
					...
					if(typeof metas[prop][0].$ !== 'undefined') {
						var name='',content='';
						html += '<meta name="'+metas[prop][0].$.name+'" content="'+metas[prop][0].$.content+'" />'+"\n";
					} else {
...
function processNavMap(epubData) {
		...
		var ops = epubData.paths.opsRoot;
		console.log('epub-parser: ops root is '+ops);
		...
		function processNavPoint(np) {
			...
			htmlNav += '<li><a href="'+ops+src+'">'+text+'</a>';
